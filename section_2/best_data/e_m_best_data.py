...
x = np.array([299.9, 290.2, 279.8, 270.5, 259.9, 250.4, 240.1, 230.4, 220.5, 210.2, 200.4])[-7:]
x_err = 0.1*np.ones(len(x))

# I is coil current I (measured using meter)

I = np.array([2.370, 2.311, 2.227, 2.028, 1.751, 1.678, 1.563, 1.481, 1.395, 1.275, 1.152])[-7:]
...
df.to_csv("section_2/best_data/e_m_best_data_table.csv")
...
with open("section_2/best_data/e_m_best_data_line_of_best_fit.txt", "w") as f:
    f.write(f"y=m*x+c\nm = {m} +- {m_err}\nc = {c} +- {c_err}")
...
plt.savefig("section_2/best_data/e_m_best_data_graph", dpi=200)
#plt.show()